...
from mvs_utils.ftensor import f_eye
...
class FlexibleDataSampler(MetadataReader):
    ...
    def save_imgs_at_pose(self, pos, i):
        ...
        new_rig_pose = f_eye(4, f0="awf", f1="rbf")
        new_rig_pose.rotation = torch.from_numpy( 
                        R.from_quat( rand_ori.to_numpy_array() ).as_matrix() )
        new_rig_pose.translation = torch.Tensor( pos ).to(dtype=torch.float32)
        self.frame_graph.add_or_update_pose_edge(new_rig_pose)
        ...
        for k in self.cam_to_camdata:
            ...
            # === Figure out the pose of the camera with w.r.t. awf. ===
            # Get the transform of cbf w.r.t. awf.
            frame_name    = cam_data["data"]["frame"]
            T_awf_cbf     = self.frame_graph.query_transform(f0="awf", f1=frame_name)
            # Convert the translation part to AirSim compatible format.
            translation   = T_awf_cbf.translation.cpu().numpy()
            t_position    = Vector3r( x_val=float(translation[0]), y_val=float(translation[1]), z_val=float(translation[2]) )
            # Then the rotation part.
            orientation   = R.from_matrix( T_awf_cbf.rotation.cpu().numpy() ).as_quat()
            t_orientation = Quaternionr( x_val=float(orientation[0]), y_val=float(orientation[1]), z_val=float(orientation[2]), w_val=float(orientation[3]) )
            # Compose the pose variable for AirSim.
            tpose         = Pose( t_position, t_orientation )
            # === End of calculation of the pose. ===

            start_pose = time.time()
            self.imgclient.setpose(tpose)
            print(f"For cam {k}, time to change pose was: {time.time() - start_pose:.6f}. Position: ({t_position.x_val}, {t_position.y_val}, {t_position.z_val})")
            ...
            airsim_imgs = self.read_and_format_6_image_pane(t_orientation, t_position) \
                if args.save_as_six_imgs else self.read_and_format_airsim_imgs()
            ...
            for t in cam_data["types"]:
                ...
                if k == "rig":
                    ...
                    rig_file_path = join(k, f"{i:06}")
                elif "is_rig" in cam_data:
                    rig_file_path = join(f"cam{k}", f"{i:06}")

                if rig_file_path is not None:
                    temp_rig_paths.append(rig_file_path)
    ...
    def close(self,):
        ...
        # Copyt the frame graph file.
        shutil.copy( self.args.frame_graph_path, join(self.data_dir, "frame_graph.json") )
...
def print_thick_banner(title):
    assert isinstance(title, str)

    # Get the length of title.
    len_title = len(title)

    # Comput the total length of the benner.
    total_length = 4 + len_title + 4

    # Compose the middle line.
    middle_line = f'=== {title} ==='

    # Compose top and bottom lines.
    tb_line = ''.join( ['='] * total_length )
...
def collect_data_from_requests(args):
    ...
    # If it is not empty then it will be used instead of the trajectory folder as the output directory.
    out_dir_override = args.collection_out_dir_override
    ...
    with open(requests_path) as requests_file:
        ...
        for envname in requests:
            ...
            for trajpath in [p for p in os.listdir(trajdir_path) if p.startswith("P") and
                ...
                data_path = join(output_path, trajpath.split('.')[0])
                ...
                # Print info.
                print_thick_banner( f'Data collected to {data_path}.' )
