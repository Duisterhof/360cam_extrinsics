...
def read_camera_config(fn):
    ...
    camera_list = []
    for i, cc in enumerate( config['value0']['T_imu_cam'] ):
        t = np.array( 
            [ cc['px'], cc['py'], cc['pz'] ], 
            dtype=np.float32 )
        
        q = np.array(
            [ cc['qx'], cc['qy'], cc['qz'], cc['qw'] ],
            dtype=np.float32 )
...
def convert_to_ned(camera_list: List[dict]):
    '''
    The frame of the sphere-stereo model is typical Camera Image Frame, 
...
if __name__ == '__main__':
    camera_config_fn = './data/sphere-stereo/calibration.json'
    out_dir = './output/sphere-stereo'
