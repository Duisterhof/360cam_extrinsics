...
def trajectory_filter(anafolder, datafolder, trajfolder):
    filename = trajfolder + '_' + datafolder + '_disp_'
    maxfile = anafolder + '/' + filename + 'max.npy'
    meanfile = anafolder + '/' + filename + 'mean.npy'
    MIN_MAXDISP = 100 # no more close than 2m
    MIN_MEANDISP = 20 # average more than 8m
    PERCENTAGE = 0.6
    if isfile(maxfile) and isfile(meanfile):
        maxdisp = np.load(maxfile)
        meandisp = np.load(meanfile)

        goodnum1 = np.sum(maxdisp < MAX_MAXDISP)
        goodnum2 = np.sum(meandisp < MAX_MEANDISP)

        trajlen = maxdisp.shape[0]
        print('    Trajlen {}, mean<10 {}, max<50 {}..'.format(trajlen, goodnum2, goodnum1))
        if goodnum1 > PERCENTAGE * trajlen or goodnum2 > PERCENTAGE * trajlen:
            return True
        else:
            return False
    else:
...
if __name__ == "__main__":
    ...
    for env_folder in env_folders:
        ...
        # read analystic files 
        anafolder = env_dir + '/' + 'analyze'
        ...
        for data_folder in data_folders:
            ...
            for trajfolder in trajfolders:
                ...
                # if trajectory_filter(anafolder, data_folder, trajfolder)==False: # skip trajectory if it does not meet certain criterions
                #     print('    Skip trajectory {}..'.format(trajfolder))
                #     continue
                print('    Working on trajectory {}..'.format(trajfolder))
                ...
                #     print('Flow folder exists, create new ' + flowpath)
                    ...
                    logf.logline('Flow folder exists, create new ' + flowpath)
                    print('Flow folder exists, skip ' + flowpath)
                    continue
