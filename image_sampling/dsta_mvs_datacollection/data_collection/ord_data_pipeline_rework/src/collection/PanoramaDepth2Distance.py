...
# Find the illustration of the coordinate system from
...
def meshgrid_from_img(img):
    ...
    # After changing to the new pixel coordiante definition.
...
def depth_2_distance(pn_depth, xx, yy, pn_dist):
    ...
    # After changing to the new pixel coordiante definition.
    xx = xx / W * ( 2 * np.pi ) # Longitute.
    yy = yy / H * np.pi       # Latitute.
...
def cuda_depth_2_distance(pn_depth, pn_dist):
    ...
    cuda.synchronize()
    k_depth_2_distance[[int(W/16), int(H/16), 1], [16, 16, 1]](d_pn_depth, d_pn_dist, res)
    cuda.synchronize()
